...
export default function HomeScreen() {
    ...
    try {
      ...
      if (savedVisibility) {
        ...
        const defaultVisibility = {
          ...
          ARB: true, MATIC: true, OP: true, USDT: true, TRX: true
        };
  ...
  // Utility to format balance in decimal notation
  ...
  // Filter wallets based on visibility settings - only show if explicitly set to true
  const visibleWallets = isVisibilityLoaded 
    ? wallets.filter(wallet => coinVisibility[wallet.network] === true)
    : wallets; // Show all wallets while loading
          ...
          {visibleWallets.length === 0 ? (
            ...
            <View>
              {visibleWallets.map((wallet) => {
                ...
                // Default decimals to 6 for all coins
                const decimals = 6;
                return (
                  <TouchableOpacity
                    key={wallet.id}
                  ...
                  >
                    ...
                    <View style={styles.walletRight}>
                      <Text variant="titleMedium" style={styles.walletBalance}>
                        {isBalanceVisible ? `${formatBalance(wallet.balance, decimals)} ${wallet.network}` : '******'}
                      </Text>
